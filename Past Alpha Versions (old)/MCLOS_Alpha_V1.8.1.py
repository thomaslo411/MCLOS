...
def render_welcome():
    ...
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Alpha V1.8.1")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Alpha V1.8.1")
...
""")
    ...
    with c3:
        st.info(f"Current seed: `{st.session_state.seed}`")
        ...
        if "error" not in out:
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v181_seed{run_seed}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v181_seed{run_seed}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])

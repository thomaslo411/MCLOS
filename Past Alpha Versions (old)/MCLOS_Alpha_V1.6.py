...
# =============================================================================
# MCLOS Alpha V1.6 - Modular Cargo Loading Optimization Software
# =============================================================================
# Changelog from V1.5:
#   - NEW: Axle load analysis
#     * Standard 5-axle tractor-trailer model (steer, drive tandem, trailer tandem)
#     * Axle positions from FHWA/DOT standards for 53-ft and 42-ft trailers
#     * Moment-balance (lever arm) physics for load distribution
#     * Per-axle weight, utilization %, and over/under limit warnings
#   - NEW: Axle visualization in 3D view
#     * Red lines show drive tandem position, blue lines show trailer tandem
#     * Axle markers with labels on the trailer floor
#   - NEW: Axle load results card (below weight distribution)
#     * Steer axle, drive tandem, trailer tandem loads + limits
#     * Color-coded utilization bars (green/yellow/red)
#   - Axle data included in JSON and CSV exports
#   - Federal weight limits: 12,000 steer / 34,000 drive / 34,000 trailer tandem
#   - Trailer presets now include axle positions + tractor weight
# =============================================================================
...
def pack_panels_v14(panel_list, trailer_L, trailer_W, trailer_H,
                    ...
                    step, seed, strategy, stress_config, axle_config=None):
    """
    ...
    for i, panel in enumerate(placed):
        ...
        # Effective compression: total load (self + everything above) / footprint
        dx, dy, dz = panel["size"]
        footprint = dx * dy
        total_load = panel["weight"] + wa
        eff_comp = total_load / footprint if footprint > 0.01 else float('inf')
        panel["stress_analysis"] = {
            "compression_psi": round(comp, 2),
            "effective_compression_psi": round(eff_comp, 4),
    ...
    # ── Step 6: Identify worst-case stress panel ──
    worst_panel = None
    worst_eff_comp = 0.0
    for i, panel in enumerate(placed):
        ec = panel["stress_analysis"]["effective_compression_psi"]
        if ec > worst_eff_comp:
            worst_eff_comp = ec
            worst_panel = {
                "index": i,
                "label": panel["label"],
                ...
                "effective_compression_psi": panel["stress_analysis"]["effective_compression_psi"],
                "shear_psi": panel["stress_analysis"]["shear_psi"],
                "total_load_lb": panel["stress_analysis"]["total_load_lb"],
                "weight_above_lb": panel["stress_analysis"]["weight_above_lb"],
                "own_weight_lb": panel["weight"],
                "footprint_sqin": panel["stress_analysis"]["footprint_sqin"],
                ...
                "compression_utilization_pct": round(
                    ec * stress_config.safety_factor / stress_config.max_compression_psi * 100, 1
                ) if stress_config.max_compression_psi > 0 else 0,
                "shear_utilization_pct": round(
                    panel["stress_analysis"]["shear_psi"] * stress_config.safety_factor
                    / stress_config.max_shear_psi * 100, 1
                ) if stress_config.max_shear_psi > 0 else 0,
            }
...
def visualize(out, stress_config, color_by_stress, show_stress_markers, show_axles, floor_spec):
    ...
    for p in out["placements"]:
        ...
        if color_by_stress:
            c = get_stress_color(
                p["stress_analysis"]["effective_compression_psi"],
                stress_config.max_compression_psi
            )
    ...
    if show_axles and axle_data:
        ...
        # Drive tandem (red) — two axle lines across the trailer width
        d_center = ap["drive_tandem_x"]
        d_half = ap["drive_tandem_spacing"] / 2.0
        for ax_x, label in [(d_center - d_half, "Drive Axle 1"), (d_center + d_half, "Drive Axle 2")]:
            # Only draw if within visible range (may be negative = off the front)
            fig.add_trace(go.Scatter3d(
                x=[ax_x, ax_x], y=[0, W], z=[0, 0],
                mode="lines", line=dict(color="#e74c3c", width=6),
                name=label, showlegend=(label == "Drive Axle 1")
            ))
        # Drive tandem center marker
        fig.add_trace(go.Scatter3d(
            x=[d_center], y=[W / 2], z=[0],
            mode="markers+text",
            ...
            name="Drive Tandem", showlegend=False
        ))

        # Trailer tandem (blue) — two axle lines across the trailer width
        t_center = ap["trailer_tandem_x"]
        t_half = ap["trailer_tandem_spacing"] / 2.0
        for ax_x, label in [(t_center - t_half, "Trailer Axle 1"), (t_center + t_half, "Trailer Axle 2")]:
            fig.add_trace(go.Scatter3d(
                x=[ax_x, ax_x], y=[0, W], z=[0, 0],
                mode="lines", line=dict(color="#3498db", width=6),
                name=label, showlegend=(label == "Trailer Axle 1")
            ))
        # Trailer tandem center marker
        fig.add_trace(go.Scatter3d(
            x=[t_center], y=[W / 2], z=[0],
            mode="markers+text",
            ...
            name="Trailer Tandem", showlegend=False
        ))
    ...
    if out["placements"]:
        ...
        with c4:
            # Show worst-case effective compression (total load / area), not self-only
            if wsp:
                st.metric("Max Eff. Comp.", f"{wsp['effective_compression_psi']:.2f} psi",
                          help="Effective compression on worst-case panel = total load / footprint")
            else:
                st.metric("Max Eff. Comp.", "N/A")
        with c5:
        ...
        if show_stress_markers and wsp:
            ...
            with ws3:
                st.metric("Eff. Compression", f"{wsp['effective_compression_psi']:.2f} psi")
                util_c = wsp["compression_utilization_pct"]
                color_fn = st.success if util_c < 50 else st.warning if util_c < 80 else st.error
                color_fn(f"Utilization: {util_c:.1f}% of {wsp['compression_limit_psi']} psi limit")
            with ws4:
                st.metric("Shear", f"{wsp['shear_psi']:.2f} psi")
                util_s = wsp["shear_utilization_pct"]
                color_fn = st.success if util_s < 50 else st.warning if util_s < 80 else st.error
                color_fn(f"Utilization: {util_s:.1f}% of {wsp['shear_limit_psi']} psi limit")
            st.caption(
                f"Position: ({wsp['position'][0]:.0f}\", {wsp['position'][1]:.0f}\", {wsp['position'][2]:.0f}\") "
                f"| Footprint: {wsp['footprint_sqin']:,.0f} sq.in."
            )
...
st.caption("HexHomes Panel Packing Optimizer  |  Alpha V1.6")
...
with st.sidebar:
    ...
    color_by_stress = st.checkbox("Color by Stress Level", value=False,
                                   help="Colors all panels green/yellow/red by effective compression")
...
if tp in AXLE_CONFIGS:
    ...
    axle_config = AxleConfig(
        drive_tandem_x=-36.0, drive_tandem_spacing=49.0,
        trailer_tandem_x=round(tL * 0.75, 1), trailer_tandem_spacing=49.0,
    )
        ...
        if "error" not in out:
            ...
            if out["placements"]:
                with st.expander("Detailed Stress Analysis"):
                    sd = [{
                        ...
                        "Layer": p["layer"],
                        "Eff Comp (psi)": p["stress_analysis"]["effective_compression_psi"],
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v16_seed{run_seed}.json", "application/json", use_container_width=True)
            with d2:
                hdr = "id,label,type,x,y,z,dx,dy,dz,orient,weight,layer,load_order,eff_comp,total_load,wt_above,shear,bend,defl\n"
                rows = "\n".join([
                    ...
                    f"{p['stress_analysis']['effective_compression_psi']},"
                    f"{p['stress_analysis']['total_load_lb']},{p['stress_analysis']['weight_above_lb']},"
                    f"{p['stress_analysis']['shear_psi']},{p['stress_analysis']['bending_stress']},"
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v16_seed{run_seed}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])

...
def render_welcome():
    ...
    st.caption("HexHomes Panel Packing Optimizer  |  Alpha V1.7")
...
""")
    ...
    with c2:
        if st.button("🎲 New random seed", use_container_width=True):
            st.session_state.seed = random.randint(0, 99999)
            st.toast(f"New seed: {st.session_state.seed}")
    with c3:
        st.info(f"Current seed: `{st.session_state.seed}`", icon="✅")

    with st.sidebar:
        st.header("MCLOS")
        st.write("Click start to enter the optimizer UI.")
        if st.button("Start MCLOS", use_container_width=True):
            st.session_state.started = True
            st.rerun()

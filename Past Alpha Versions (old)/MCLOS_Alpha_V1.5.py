...
# =============================================================================
# MCLOS Alpha V1.5 - Modular Cargo Loading Optimization Software
# =============================================================================
# Changelog from V1.4:
#   - NEW: Effective compression metric per panel
#     * effective_compression_psi = (own weight + all weight above) / footprint
#     * This is the TRUE compressive load seen by each panel
#   - NEW: Worst-case stress panel identification
#     * Output includes worst_stress_panel with full breakdown
#     * 3D marker highlights the most-stressed panel (yellow X)
#   - NEW: Toggleable "Show Stress Markers" in sidebar
#     * When ON: worst-case panel highlighted in 3D + stress info card
#     * Shows effective compression, shear, total load, safety margin
#   - Updated summary metrics to show effective compression (not self-only)
# =============================================================================
...
def visualize(out, stress_config, color_by_stress, show_stress_markers, floor_spec):
    if "error" in out:
        st.error(out["error"])
        return
...
st.caption("HexHomes Panel Packing Optimizer  |  Alpha V1.5")
...
if run_btn:
    ...
    if not panel_list:
        ...
        if "error" not in out:
            ...
            visualize(out, stress_config, color_by_stress, show_stress_markers, floor_spec)
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v15_seed{run_seed}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v15_seed{run_seed}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])

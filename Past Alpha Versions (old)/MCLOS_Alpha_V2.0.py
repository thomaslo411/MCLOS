...
def render_welcome():
    ...
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.0")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.0")
        ...
        if "error" not in out:
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v20_run{st.session_state.run_count}.json", "application/json", use_container_width=True)
            with d2:
                hdr = "id,label,type,x,y,z,dx,dy,dz,orient,weight,layer,load_order,comp_psi,bend_psi,shear_psi,defl_in,util_ratio,limiting,total_load,wt_above\n"
                rows = "\n".join([
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v20_run{st.session_state.run_count}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])

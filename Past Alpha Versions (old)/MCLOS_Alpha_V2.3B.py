...
def render_welcome():
    ...
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.2.1")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.2.1")
        ...
        if "error" not in out:
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v221_run{st.session_state.run_count}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v221_run{st.session_state.run_count}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])

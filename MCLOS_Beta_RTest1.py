...
def pack_panels_v14(panel_list, trailer_L, trailer_W, trailer_H,
    ...
    for panel_info in panel_list:
        ...
        for name, size, axis_map in raw:
            ...
            if is_flatbed and not panel_is_flat:
                continue
    ...
    def _flatbed_priority(pt):
        if pt == PanelType.FLOOR:
            return 0
        if pt in (PanelType.ROOF_FRAME_MID, PanelType.ROOF_FRAME_TRI):
            return 1
        if pt in (PanelType.ROOF_FLAT_RECT, PanelType.ROOF_FLAT_TRI):
            return 2
...
def render_welcome():
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Beta 1 — Roof Sub-Panels")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Beta 1 — Roof Sub-Panels")
        ...
        if "error" not in out:
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_beta1_run{st.session_state.run_count}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_beta1_run{st.session_state.run_count}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])
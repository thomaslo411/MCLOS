...
class PanelType(Enum):
    ...
    ROOF_FRAME_MID = "Roof Frame Middle"
    ROOF_FRAME_TRI = "Roof Frame Triangle"
    ROOF_FLAT_RECT = "Roof Flat Rectangular"
    ROOF_FLAT_TRI = "Roof Flat Triangular"


ROOF_PANEL_TYPES = {PanelType.ROOF_FRAME_MID, PanelType.ROOF_FRAME_TRI,
                    PanelType.ROOF_FLAT_RECT, PanelType.ROOF_FLAT_TRI}
...
class PanelSpec:
    ...
    def is_triangle(self):
        return self.panel_type in (PanelType.ROOF_FLAT_TRI, PanelType.ROOF_FRAME_TRI)
...
ROOF_FRAME_MID_DEFAULT = PanelSpec(
    panel_type=PanelType.ROOF_FRAME_MID, length=194.0, height=112.0, thickness=24.74,
    weight=358.70, short_edge=0.0
)
ROOF_FRAME_TRI_DEFAULT = PanelSpec(
    panel_type=PanelType.ROOF_FRAME_TRI, length=112.5, height=56.0, thickness=32.0,
    weight=38.12, short_edge=0.0
)
ROOF_FLAT_RECT_DEFAULT = PanelSpec(
    panel_type=PanelType.ROOF_FLAT_RECT, length=111.52, height=102.0, thickness=0.44,
    weight=180.77, short_edge=0.0
)
ROOF_FLAT_TRI_DEFAULT = PanelSpec(
    panel_type=PanelType.ROOF_FLAT_TRI, length=115.15, height=64.0, thickness=0.44,
    weight=51.42, short_edge=0.0
)

ROOF_DEFAULTS = {
    PanelType.ROOF_FRAME_MID: ROOF_FRAME_MID_DEFAULT,
    PanelType.ROOF_FRAME_TRI: ROOF_FRAME_TRI_DEFAULT,
    PanelType.ROOF_FLAT_RECT: ROOF_FLAT_RECT_DEFAULT,
    PanelType.ROOF_FLAT_TRI: ROOF_FLAT_TRI_DEFAULT,
}

ROOF_WEIGHT_PER_POD = (
    1 * ROOF_FRAME_MID_DEFAULT.weight +
    2 * ROOF_FRAME_TRI_DEFAULT.weight +
    2 * ROOF_FLAT_RECT_DEFAULT.weight +
    4 * ROOF_FLAT_TRI_DEFAULT.weight
)
...
def in_bounds_with_overhang(pos, size, trailer, is_flatbed=False,
                             overhang_max=0.0, panel_allows_overhang=False):
    x, y, z = pos
    dx, dy, dz = size
    tL, tW, tH = trailer
    ...
    if is_flatbed and panel_allows_overhang and overhang_max > 0:
...
def get_triangle_footprint_2d(pos, size, axis_map):
    """Return 3-vertex 2D polygon for a triangular roof panel lying flat."""
    x, y, _ = pos
    dx, dy, _ = size
    x_axis = axis_map[0]

    if x_axis == "L":
        # Base along X (full width dx), apex at midpoint of opposite edge
        return [(x, y), (x + dx, y), (x + dx / 2.0, y + dy)]
    elif x_axis == "H":
        # Base along Y (full height dy), apex at midpoint of opposite edge
        return [(x, y), (x, y + dy), (x + dx, y + dy / 2.0)]
    # Fallback: same as L-along-X
    return [(x, y), (x + dx, y), (x + dx / 2.0, y + dy)]
...
def _build_footprint_polygon(pos, size, info):
    """Build 2D footprint polygon for a panel: trapezoid, triangle, or rectangle."""
    is_trap = (info.get('is_trapezoid', False) and
               info.get('axis_map', ('', '', ''))[2] == 'T')
    is_tri = (info.get('is_triangle', False) and
              info.get('axis_map', ('', '', ''))[2] == 'T')

    if is_trap:
        return get_trapezoid_footprint_2d(
            pos, size, info['axis_map'],
            info['short_edge'], info['spec_length'],
            info.get('flipped', False))
    elif is_tri:
        return get_triangle_footprint_2d(pos, size, info['axis_map'])
    else:
        x, y = pos[0], pos[1]
        dx, dy = size[0], size[1]
        return [(x, y), (x + dx, y), (x + dx, y + dy), (x, y + dy)]
...
def panels_collide(p1_pos, p1_size, p1_info, p2_pos, p2_size, p2_info):
    ...
    p1_flat_tri = (p1_info.get('is_triangle', False) and
                   p1_info.get('axis_map', ('', '', ''))[2] == 'T')
    p2_flat_tri = (p2_info.get('is_triangle', False) and
                   p2_info.get('axis_map', ('', '', ''))[2] == 'T')

    uses_polygon = (p1_flat_trap or p2_flat_trap or p1_flat_tri or p2_flat_tri)
    ...
    if uses_polygon:
        ...
        poly1 = _build_footprint_polygon(p1_pos, p1_size, p1_info)
        poly2 = _build_footprint_polygon(p2_pos, p2_size, p2_info)
...
def calculate_bending_stress(panel_weight, pos, size, supporting_panels):
    ...
    if len(sx_positions) >= 2:
        ...
        sp = supporting_panels[0]['panel']
        sp_x_min = sp['pos'][0]
        sp_x_max = sp['pos'][0] + sp['size'][0]
        overhang_left = max(0.0, sp_x_min - x)
        overhang_right = max(0.0, (x + dx) - sp_x_max)
        overhang_x = max(overhang_left, overhang_right)
        if overhang_x > 1e-3:
            w_x = panel_weight / dx if dx > 1e-3 else 0.0
    ...
    if len(sy_positions) >= 2:
        ...
        sp = supporting_panels[0]['panel']
        sp_y_min = sp['pos'][1]
        sp_y_max = sp['pos'][1] + sp['size'][1]
        overhang_top = max(0.0, sp_y_min - y)
        overhang_bot = max(0.0, (y + dy) - sp_y_max)
        overhang_y = max(overhang_top, overhang_bot)
        if overhang_y > 1e-3:
            w_y = panel_weight / dy if dy > 1e-3 else 0.0
...
def calculate_deflection(panel_weight, pos, size, youngs_modulus, supporting_panels):
    ...
    if len(sx_positions) >= 2:
        ...
        sp_x_min = sp['pos'][0]
        sp_x_max = sp['pos'][0] + sp['size'][0]
        overhang_x = max(max(0.0, sp_x_min - x), max(0.0, (x + dx) - sp_x_max))
        if overhang_x > 1e-3:
            w_x = panel_weight / dx if dx > 1e-3 else 0.0
            I_x = (dy * dz ** 3) / 12.0
            if I_x > 1e-6 and youngs_modulus > 1e-3:
    ...
    if len(sy_positions) >= 2:
        ...
        sp_y_min = sp['pos'][1]
        sp_y_max = sp['pos'][1] + sp['size'][1]
        overhang_y = max(max(0.0, sp_y_min - y), max(0.0, (y + dy) - sp_y_max))
        if overhang_y > 1e-3:
            w_y = panel_weight / dy if dy > 1e-3 else 0.0
            I_y = (dx * dz ** 3) / 12.0
            if I_y > 1e-6 and youngs_modulus > 1e-3:
...
def generate_orientations_roof_frame(L, H, T):
    """Orientations for roof frame panels — standing upright preferred.
    Standing orientations listed first so they are tried before flat fallbacks."""
    return [
        ("stand_LxT", (L, T, H), ("L", "T", "H")),
        ("stand_TxL", (T, L, H), ("T", "L", "H")),
        ("stand_HxT", (H, T, L), ("H", "T", "L")),
...
def _add_roof_panels_for_pod(panels, pod_label, roof_specs):
    """Add all 9 roof sub-panels for one pod (or roof set)."""
    fm = roof_specs[PanelType.ROOF_FRAME_MID]
    ft = roof_specs[PanelType.ROOF_FRAME_TRI]
    fr = roof_specs[PanelType.ROOF_FLAT_RECT]
    ftr = roof_specs[PanelType.ROOF_FLAT_TRI]

    panels.append({"spec": fm, "label": f"{pod_label}_RoofFrameMid",
                    "panel_type": PanelType.ROOF_FRAME_MID.value})
    for i in range(2):
        panels.append({"spec": ft, "label": f"{pod_label}_RoofFrameTri_{i+1}",
                        "panel_type": PanelType.ROOF_FRAME_TRI.value})
    for i in range(2):
        panels.append({"spec": fr, "label": f"{pod_label}_RoofFlatRect_{i+1}",
                        "panel_type": PanelType.ROOF_FLAT_RECT.value})
    for i in range(4):
        panels.append({"spec": ftr, "label": f"{pod_label}_RoofFlatTri_{i+1}",
                        "panel_type": PanelType.ROOF_FLAT_TRI.value})


def build_panel_list_from_pods(num_pods, wall_spec, floor_spec, roof_specs):
    panels = []
    ...
    for pod in range(num_pods):
        _add_roof_panels_for_pod(panels, f"Pod{pod + 1}", roof_specs)
    for pod in range(num_pods):
        for wall in range(6):
            panels.append({
                "spec": wall_spec,
...
def build_panel_list_manual(num_walls, num_floors, num_roof_sets, wall_spec, floor_spec, roof_specs):
    panels = []
    ...
    for i in range(num_roof_sets):
        _add_roof_panels_for_pod(panels, f"RoofSet{i + 1}", roof_specs)
...
def pack_panels_v14(panel_list, trailer_L, trailer_W, trailer_H,
    ...
    for panel_info in panel_list:
        ...
        is_roof_frame = spec.panel_type in (PanelType.ROOF_FRAME_MID, PanelType.ROOF_FRAME_TRI)
        if spec.is_trapezoid:
            raw = generate_orientations_floor(bbox[0], bbox[1], bbox[2])
        elif is_roof_frame:
            raw = generate_orientations_roof_frame(bbox[0], bbox[1], bbox[2])
        else:
            raw = generate_orientations_wall(bbox[0], bbox[1], bbox[2])
        valid = []
        for name, size, axis_map in raw:
            panel_is_flat = (axis_map[2] == 'T')
            panel_allows_ovh = (is_flatbed and panel_is_flat)
            if not in_bounds_with_overhang((0, 0, 0), size, trailer,
                                           is_flatbed, overhang_max_in, panel_allows_ovh):
                continue
            if not handling_ok(size, max_horizontal, max_vertical):
                continue

            # On flatbed: only flat orientations allowed, EXCEPT roof frames can stand
            if is_flatbed and not panel_is_flat and not is_roof_frame:
                continue
            ...
            if spec.is_trapezoid and name.startswith("flat_") and is_flatbed:
                ...
                ovh = overhang_max_in if panel_allows_ovh else 0.0
            ...
            # Roof frames: cap at 1 layer (don't stack frames on top of each other)
            if is_roof_frame:
                max_stack = 1
    ...
    def _flatbed_priority(pt):
        """Practical loading order: floors first, frames upright, walls fill deck, flats on top."""
        if pt == PanelType.FLOOR:
            return 0          # 585 lb — heaviest, tessellated on deck
        if pt == PanelType.ROOF_FRAME_MID:
            return 1          # 358 lb — stands upright, minimal deck footprint
        if pt == PanelType.ROOF_FRAME_TRI:
            return 2          # 38 lb — stands upright in gaps
        if pt == PanelType.WALL:
            return 3          # 220 lb — flat on deck, fills remaining space
        if pt == PanelType.ROOF_FLAT_RECT:
            return 4          # 181 lb — light flat, placed on top of stacks
        if pt == PanelType.ROOF_FLAT_TRI:
            return 5          # 51 lb — lightest, goes on very top
        return 6

    if is_flatbed:
        sorted_panels = sorted(panel_list, key=lambda p: (
            _flatbed_priority(p['spec'].panel_type),
    ...
    for pid, panel_info in enumerate(sorted_panels):
        ...
        if not placed_this and placed:
            for orient in orientations:
                ...
                axis_map = orient['axis_map']
                dx, dy, dz = size
                panel_is_flat_ms = (axis_map[2] == 'T')
                panel_allows_ovh_ms = (is_flatbed and panel_is_flat_ms)
                ...
                for base in placed:
                    ...
                    for (tx, ty) in candidates:
                        ...
                        if not in_bounds_with_overhang(test_pos, size, trailer,
                                                       is_flatbed, overhang_max_in, panel_allows_ovh_ms):
                            continue
    ...
    roof_count = sum(1 for p in placed if p["panel_type"] in {pt.value for pt in ROOF_PANEL_TYPES})
...
def make_triangle_prism_vertices(x, y, z, dx, dy, dz, axis_map):
    """Build 6 vertices for a triangular prism (3 bottom + 3 top).
    Isosceles triangle: base along one axis, apex at midpoint of opposite edge."""
    x_axis = axis_map[0]
    if x_axis == "L":
        b0 = (x, y, z)
        b1 = (x + dx, y, z)
    ...
    t1 = (b1[0], b1[1], z + dz)
    t2 = (b2[0], b2[1], z + dz)
    return [b0, b1, b2, t0, t1, t2]


def add_triangle_prism_solid(fig, verts, color, opacity=0.45):
    """Render a triangular prism as Mesh3d. 6 vertices, 8 triangular faces."""
    vx = [v[0] for v in verts]
    vy = [v[1] for v in verts]
    vz = [v[2] for v in verts]
    # Faces: bottom(0,1,2), top(3,4,5), side1(0,1,4)(0,4,3), side2(1,2,5)(1,5,4), side3(2,0,3)(2,3,5)
...
def add_triangle_prism_wire(fig, verts, color, name="", opacity=1.0, width=4):
    """Wireframe for triangular prism: 3 bottom + 3 top + 3 vertical edges = 9."""
...
def visualize(out, stress_config, color_by_stress, show_stress_markers, show_axles, floor_spec,
    ...
    roof_frame_pal = ["#8e44ad", "#6c3483", "#a569bd", "#7d3c98"]
    roof_flat_pal = ["#c39bd3", "#d2b4de", "#bb8fce", "#af7ac5"]
    _roof_type_values = {pt.value for pt in ROOF_PANEL_TYPES}
    _roof_frame_values = {PanelType.ROOF_FRAME_MID.value, PanelType.ROOF_FRAME_TRI.value}
    wi, fi, rfi, rpi = 0, 0, 0, 0
    tc = {}
    ...
    for p in out["placements"]:
        ...
        if key not in tc:
            ...
            elif pt in _roof_frame_values:
                tc[key] = roof_frame_pal[rfi % len(roof_frame_pal)]
                rfi += 1
            elif pt in _roof_type_values:
                tc[key] = roof_flat_pal[rpi % len(roof_flat_pal)]
                rpi += 1
            else:
        ...
        elif p.get("is_triangle", False) and tuple(p.get("axis_map", ["L", "H", "T"]))[2] == 'T':
            am = tuple(p.get("axis_map", ["L", "H", "T"]))
            verts = make_triangle_prism_vertices(x, y, z, dx, dy, dz, am)
            add_triangle_prism_solid(fig, verts, c, opacity=0.45)
            add_triangle_prism_wire(fig, verts, c, "", opacity=wire_opacity, width=wire_width)
        else:
    ...
    if not color_by_stress:
        ...
        for key, c in tc.items():
            ...
            if "Floor" in pt:
                short = "Floor"
            elif "Frame Mid" in pt:
                short = "RoofFrMid"
            elif "Frame Tri" in pt:
                short = "RoofFrTri"
            elif "Flat Rect" in pt:
                short = "RoofFlRect"
            elif "Flat Tri" in pt:
                short = "RoofFlTri"
            elif "Roof" in pt:
...
def render_welcome():
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Beta 2 — Triangle Shapes & Practical Loading")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Beta 2 — Triangle Shapes & Practical Loading")
...
This app packs **HexHomes wall, floor, and roof panels** into flatbed trailer presets (or custom dimensions), then reports:

- **3D load visualization** (panels, CG marker, and **3D axles**)
- **Structural checks** (support area, compression, bending, shear, deflection)
- **Auto-optimized loading** (tests multiple strategies, picks the best)
- **Axle-load analysis** (steer / drive tandem / trailer tandem vs DOT limits)
- **Roof sub-panels**: 9 pieces per pod (frame middle, frame triangles, flat panels)
...
if is_flatbed:
    overhang_max_in = st.number_input(
        ...
        help="Panels can extend past deck width on flatbed trailers (no walls). "
...
if input_mode == "By Pod Count (quick)":
    ...
    with cp1:
        num_pods = st.number_input("Pods", min_value=1, max_value=6, value=3, step=1,
                                   help="6 walls + 2 floors + 9 roof pieces per pod")
    with cp2:
        tw = num_pods * 6
        tf = num_pods * 2
        tr_ = num_pods * 9
        total_p = tw + tf + tr_
        st.markdown(f"**{num_pods} Pod{'s' if num_pods > 1 else ''}** = **{total_p} panels** ({tw}W + {tf}F + {tr_}R)")
        st.caption(f"Roof per pod: 1 frame mid + 2 frame tri + 2 flat rect + 4 flat tri")
    use_pods = True
else:
    ...
    with mc3:
        manual_roof_sets = st.number_input("Roof Sets", min_value=0, value=3, step=1, key="mrs",
                                            help="Each roof set = 9 pieces (1 frame mid + 2 frame tri + 2 flat rect + 4 flat tri)")
    with mc4:
        total_manual = manual_walls + manual_floors + manual_roof_sets * 9
        st.metric("Total", total_manual)
    use_pods = False
...
with st.expander("Panel Dimensions (edit if needed)"):
    ...
    st.markdown("---")
    st.markdown("**Roof Sub-Panels** (9 per pod)")
    rc1, rc2, rc3, rc4 = st.columns(4)
    with rc1:
        st.markdown("*Frame Middle* (x1)")
        rfm_L = st.number_input("L (in)", value=ROOF_FRAME_MID_DEFAULT.length, min_value=1.0, key="rfml")
        rfm_H = st.number_input("H (in)", value=ROOF_FRAME_MID_DEFAULT.height, min_value=1.0, key="rfmh")
        rfm_T = st.number_input("T (in)", value=ROOF_FRAME_MID_DEFAULT.thickness, min_value=0.1, key="rfmt")
        rfm_W = st.number_input("Wt (lb)", value=ROOF_FRAME_MID_DEFAULT.weight, min_value=0.1, key="rfmw")
    with rc2:
        st.markdown("*Frame Triangle* (x2)")
        rft_L = st.number_input("L (in)", value=ROOF_FRAME_TRI_DEFAULT.length, min_value=1.0, key="rftl")
        rft_H = st.number_input("H (in)", value=ROOF_FRAME_TRI_DEFAULT.height, min_value=1.0, key="rfth")
        rft_T = st.number_input("T (in)", value=ROOF_FRAME_TRI_DEFAULT.thickness, min_value=0.1, key="rftt")
        rft_W = st.number_input("Wt (lb)", value=ROOF_FRAME_TRI_DEFAULT.weight, min_value=0.1, key="rftw")
    with rc3:
        st.markdown("*Flat Rectangular* (x2)")
        rfr_L = st.number_input("L (in)", value=ROOF_FLAT_RECT_DEFAULT.length, min_value=1.0, key="rfrl")
        rfr_H = st.number_input("H (in)", value=ROOF_FLAT_RECT_DEFAULT.height, min_value=1.0, key="rfrh")
        rfr_T = st.number_input("T (in)", value=ROOF_FLAT_RECT_DEFAULT.thickness, min_value=0.01, step=0.01, key="rfrt")
        rfr_W = st.number_input("Wt (lb)", value=ROOF_FLAT_RECT_DEFAULT.weight, min_value=0.1, key="rfrw")
    with rc4:
        st.markdown("*Flat Triangular* (x4)")
        rftri_L = st.number_input("L (in)", value=ROOF_FLAT_TRI_DEFAULT.length, min_value=1.0, key="rftril")
        rftri_H = st.number_input("H (in)", value=ROOF_FLAT_TRI_DEFAULT.height, min_value=1.0, key="rftrih")
        rftri_T = st.number_input("T (in)", value=ROOF_FLAT_TRI_DEFAULT.thickness, min_value=0.01, step=0.01, key="rftrit")
        rftri_W = st.number_input("Wt (lb)", value=ROOF_FLAT_TRI_DEFAULT.weight, min_value=0.1, key="rftriw")
...
roof_specs = {
    PanelType.ROOF_FRAME_MID: PanelSpec(panel_type=PanelType.ROOF_FRAME_MID,
                                         length=rfm_L, height=rfm_H, thickness=rfm_T, weight=rfm_W),
    PanelType.ROOF_FRAME_TRI: PanelSpec(panel_type=PanelType.ROOF_FRAME_TRI,
                                         length=rft_L, height=rft_H, thickness=rft_T, weight=rft_W),
    PanelType.ROOF_FLAT_RECT: PanelSpec(panel_type=PanelType.ROOF_FLAT_RECT,
                                         length=rfr_L, height=rfr_H, thickness=rfr_T, weight=rfr_W),
    PanelType.ROOF_FLAT_TRI: PanelSpec(panel_type=PanelType.ROOF_FLAT_TRI,
                                        length=rftri_L, height=rftri_H, thickness=rftri_T, weight=rftri_W),
}
...
if run_btn:
    if use_pods:
        panel_list = build_panel_list_from_pods(num_pods, wall_spec, floor_spec, roof_specs)
    else:
        panel_list = build_panel_list_manual(manual_walls, manual_floors, manual_roof_sets, wall_spec, floor_spec, roof_specs)
        ...
        if "error" not in out:
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_beta2_run{st.session_state.run_count}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_beta2_run{st.session_state.run_count}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])
...
#V2.3B — Cosmetic tractor/cab 3D visualization (hood, cab body, roof, frame rails, 5th wheel plate, bumper bar, exhaust stacks). Toggle via sidebar checkbox.
#V2.3C — Light/Dark theme toggle. Light mode: white background, #FFB81C orange accents for subheadings/buttons, dark text. Dark mode = existing HexHomes Honey/Gold theme (default).
#V2.3D — Light mode CSS fixes: stress analysis cards, JSON viewer, advanced settings/panel dimensions readability, alert boxes, expander internals, dropdown arrows, tooltips, placeholders.
...
def pack_panels_v14(panel_list, trailer_L, trailer_W, trailer_H,
    ...
    V2.2.1: Cosmetic tractor/cab 3D visualization.
...
def visualize(out, stress_config, color_by_stress, show_stress_markers, show_axles, floor_spec,
              show_tractor=False, theme_colors=None):
    if "error" in out:
        st.error(out["error"])
        return
    # Resolve theme colors for Plotly chart (fall back to dark)
    tc_theme = theme_colors if theme_colors else DARK_THEME
    ...
    # ── Tractor / Cab visualization (cosmetic) ──
    ...
    if show_tractor and axle_data_for_tractor:
        ...
        drive_cx = ap_t["drive_tandem_x"]                    # -36.0 typically
        wheelbase = ap_t.get("tractor_wheelbase", 240.0)
        steer_ax = drive_cx - wheelbase                      # ~ -276.0

        # Tractor body dimensions (inches, approximate for a day cab)
        cab_width = 96.0       # standard truck body width
        hood_width = 80.0      # narrower engine hood
        frame_height = 6.0     # chassis rail depth
        hood_height = 50.0     # engine hood top
        cab_height = 100.0     # roof line
        roof_thickness = 8.0   # roof slab
        bumper_overhang = 24.0 # front bumper extension past steer axle

        # Key X positions
        bumper_x = steer_ax - bumper_overhang               # front of truck
        windshield_x = steer_ax + 80.0                      # hood-to-cab transition
        cab_back_x = drive_cx + 30.0                        # back of cab (just past drive tandem)
        fifth_wheel_x = -24.0                               # 5th wheel coupling plate

        # Y centering (center on trailer width)
        cab_y0 = (W - cab_width) / 2.0
        hood_y0 = (W - hood_width) / 2.0

        tractor_color = "#3d4f5f"        # dark steel blue
        hood_color = "#4a6274"           # slightly lighter
        frame_color = "#2c3e50"          # dark charcoal
        roof_color = "#344a5e"           # muted blue
        fifth_wh_color = "#555555"       # neutral gray

        # 1) Frame rails — two beams from bumper to trailer front
        rail_width = 4.0
        rail_y_left = (W - 36.0) / 2.0   # rails ~36" apart (standard frame)
        ...
        # 5) Fifth wheel plate (coupling point between tractor and trailer)
        plate_w = 40.0
        plate_d = 24.0
        ...
        # 7) Exhaust stacks (two thin cylinders behind cab, optional visual flair)
        stack_radius = 3.0
        stack_height = 40.0
    ...
    fig.update_layout(
        ...
        paper_bgcolor=tc_theme["plotly_paper"],
        scene=dict(
            xaxis_title="Length (in)", yaxis_title="Width (in)", zaxis_title="Height (in)",
            aspectmode="data", camera=dict(eye=dict(x=1.3, y=1.3, z=1.0)),
            bgcolor=tc_theme["plotly_scene"],
            xaxis=dict(backgroundcolor=tc_theme["plotly_scene"], gridcolor=tc_theme["plotly_grid"], color=tc_theme["plotly_axis_text"]),
            yaxis=dict(backgroundcolor=tc_theme["plotly_scene"], gridcolor=tc_theme["plotly_grid"], color=tc_theme["plotly_axis_text"]),
            zaxis=dict(backgroundcolor=tc_theme["plotly_scene"], gridcolor=tc_theme["plotly_grid"], color=tc_theme["plotly_axis_text"]),
        ),
        legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01,
                    bgcolor=tc_theme["plotly_legend_bg"], font=dict(color=tc_theme["plotly_legend_text"], size=11))
    )
...
# Shared CSS structure (selectors identical, only colors differ)
def _build_theme_css(t):
    """Build full CSS string from a theme color dictionary."""
    return f"""<style>
/* Main background */
.stApp {{
...
.stButton > button[data-testid="stBaseButton-primary"]:hover {{
    background-color: {t['btn_primary_hover']} !important;
    color: {t['btn_primary_text']} !important;
}}
/* Secondary buttons */
...
.stButton > button:not([kind="primary"]):not([data-testid="stBaseButton-primary"]):hover {{
    background-color: {t['btn_secondary_hover_bg']} !important;
    border-color: {t['accent']} !important;
}}
/* Headers */
h1 {{
...
.stDownloadButton > button {{
    background-color: {t['download_bg']} !important;
    color: {t['btn_secondary_text']} !important;
...
[data-testid="stJson"],
.stJson, pre {{
    background-color: {t['json_bg']} !important;
...
[data-testid="stPlotlyChart"] {{
    ...
    border-radius: 8px;
    padding: 4px;
    background-color: {t['plotly_paper']} !important;
}}
.modebar {{
    background-color: transparent !important;
}}
.modebar-btn path {{
    fill: {t['subheading']} !important;
}}
/* Dataframe */
.stDataFrame {{
    background-color: {t['expander_bg']} !important;
...
div[data-testid="stAlert"] svg {{
    fill: {t['secondary_text']} !important;
}}

/* ── JSON viewer — full override ── */
[data-testid="stJson"] {{
    background-color: {t['json_bg']} !important;
    color: {t['text']} !important;
    border: 1px solid {t['divider']} !important;
...
/* ── Expander internals — force text readability ── */
...
details[data-testid="stExpander"] input,
details[data-testid="stExpander"] select,
details[data-testid="stExpander"] textarea {{
    background-color: {t['input_bg']} !important;
    color: {t['text']} !important;
    border-color: {t['input_border']} !important;
}}
details[data-testid="stExpander"] [data-testid="stMetricLabel"] {{
    color: {t['caption']} !important;
}}
details[data-testid="stExpander"] [data-testid="stMetricValue"] {{
    color: {t['metric_value']} !important;
}}
...
/* ── Number input step buttons inside expanders ── */
details[data-testid="stExpander"] .stNumberInput button {{
    background-color: {t['btn_secondary_hover_bg']} !important;
    color: {t['secondary_text']} !important;
    border-color: {t['input_border']} !important;
}}
...
# ── Dark Mode (HexHomes Honey/Gold — original theme) ──
DARK_THEME = {
    "bg": "#242012", "sidebar_bg": "#1C1810", "text": "#F5F0E8",
    "sidebar_text": "#E8DABE", "accent": "#E8A817", "subheading": "#D4A832",
    "secondary_text": "#F0D68A", "caption": "#C0A870", "metric_value": "#F0D68A",
    "input_bg": "#302818", "input_border": "#4A3D28", "divider": "#3E3420",
    "expander_bg": "#1E1A12", "info_bg": "#2A2418", "json_bg": "#1A160E",
    "btn_primary_text": "#1A1408", "btn_primary_hover": "#D49A10",
    "btn_secondary_text": "#F0D68A", "btn_secondary_hover_bg": "#302818",
    "download_bg": "#302818", "scrollbar_hover": "#5A4A30",
    "plotly_paper": "#161208", "plotly_scene": "#1A160E",
    "plotly_grid": "#3E3420", "plotly_axis_text": "#D4A832",
    "plotly_legend_bg": "rgba(22,18,8,0.9)", "plotly_legend_text": "#F5F0E8",
}

# ── Light Mode (White + Orange #FFB81C) ──
LIGHT_THEME = {
    "bg": "#FFFFFF", "sidebar_bg": "#F5F5F0", "text": "#1A1A1A",
    "sidebar_text": "#333333", "accent": "#FFB81C", "subheading": "#FFB81C",
    "secondary_text": "#4A4A4A", "caption": "#666666", "metric_value": "#1A1A1A",
    "input_bg": "#FFFFFF", "input_border": "#CCCCCC", "divider": "#E0E0E0",
    "expander_bg": "#FAFAFA", "info_bg": "#FFF8EC", "json_bg": "#F5F5F5",
    "btn_primary_text": "#1A1408", "btn_primary_hover": "#E8A510",
    "btn_secondary_text": "#B88A10", "btn_secondary_hover_bg": "#FFF3D6",
    "download_bg": "#FFF3D6", "scrollbar_hover": "#AAAAAA",
    "plotly_paper": "#FFFFFF", "plotly_scene": "#F0F0F0",
    "plotly_grid": "#CCCCCC", "plotly_axis_text": "#333333",
    "plotly_legend_bg": "rgba(255,255,255,0.9)", "plotly_legend_text": "#1A1A1A",
}

DARK_CSS = _build_theme_css(DARK_THEME)
LIGHT_CSS = _build_theme_css(LIGHT_THEME)
...
_active_css = DARK_CSS if st.session_state.theme == "dark" else LIGHT_CSS
...
def render_welcome():
    ...
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.3D")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.3D")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("Display")
    _theme_options = ["Dark Mode", "Light Mode"]
    _theme_idx = 0 if st.session_state.theme == "dark" else 1
    _theme_choice = st.radio("Theme", _theme_options, index=_theme_idx,
                              horizontal=True, key="theme_radio")
    _new_theme = "dark" if _theme_choice == "Dark Mode" else "light"
...
if run_btn:
    ...
    if not panel_list:
        ...
        if "error" not in out:
            ...
            _active_theme = DARK_THEME if st.session_state.theme == "dark" else LIGHT_THEME
            visualize(out, stress_config, color_by_stress, show_stress_markers, show_axles, floor_spec,
                      show_tractor=show_tractor, theme_colors=_active_theme)
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v23d_run{st.session_state.run_count}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v23d_run{st.session_state.run_count}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])

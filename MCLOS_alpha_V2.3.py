...
# Version History: V1.0 Is starting version of the Alpha.
...
#V2.0 — 1.5" default spacer, sidebar layout fix, upgraded stress calculations (biaxial bending, parabolic shear, proper deflection with Young's modulus), utilization-based worst-panel metric, removed effective compression for regular compression (psi).
#V2.1 — Flatbed overhang support (floor panels extend past deck width), true half-hex polygon collision, tessellated nesting of half-hex panels, flipped panel tracking. Floor panels now load on flatbed trailers utilizing their geometry.
#V2.2 — Smarter flatbed arrangement: force flat orientations on flatbed (no standing without walls), block Wall First on flatbed, flatbed-aware panel/slot ordering, mixed stacking as last resort for overflow panels.
#V2.3 — Added visualization of panel spacers and semitruck.
...
def apply_strategy_slot_order(strategy, floor_slots, placed, trailer, panel_size, is_flatbed=False):
    ...
    if strategy == LoadingStrategy.GRAVITY_LAYERED:
        if is_flatbed:
            # Center-filling: prefer center of deck width, spread along length
...
def pack_panels_v14(panel_list, trailer_L, trailer_W, trailer_H,
    ...
    V2.1: Added flatbed overhang, polygon collision, tessellated floor slots.
    V2.2: Force flat on flatbed, block WALL_FIRST on flatbed, flatbed-aware ordering, mixed stacking fallback.
    ...
    for panel_info in panel_list:
        ...
        for name, size, axis_map in raw:
            ...
            # Reject standing orientations on flatbed (no walls to support upright panels)
    ...
    if is_flatbed:
        # Flatbed: floor panels first (most constrained, benefit from tessellation),
        # then walls by weight descending
        sorted_panels = sorted(panel_list, key=lambda p: (
            0 if p['spec'].panel_type == PanelType.FLOOR else 1,
    ...
    for pid, panel_info in enumerate(sorted_panels):
        ...
        # ── LAST RESORT: Mixed stacking — try placing on top of any already-placed panel ──
        if not placed_this and placed:
            for orient in orientations:
                ...
                for base in placed:
                    ...
                    # Z level on top of the base panel
                    test_z = round(bz + bdz + panel_gap_in, 4)

                    # Generate candidate (x, y) positions on top of the base:
                    # 1) Exact base position
                    # 2) Clamped to trailer bounds (for when base has overhang)
                    # 3) Centered on base, clamped to bounds
                    candidates = set()
                    ...
                    # Center new panel on base footprint, clamped
                    ...
                    for (tx, ty) in candidates:
                        ...
                        # Bounds check — overhang-aware
                        if not in_bounds_with_overhang(test_pos, size, trailer,
                                                       is_flatbed, overhang_max_in, panel_is_floor_flat):
                            continue

                        # Collision check — polygon-aware
                        panel_info_new = {
                            'is_trapezoid': spec.is_trapezoid,
                            'axis_map': axis_map,
                            'short_edge': spec.short_edge,
                        ...
                        # Update column tracker for this position
    ...
    return {
        ...
        "settings": {
            ...
            # Backward-compat key: now interpreted as spacer-per-side.
            "panel_gap_in": float(panel_gap_in) / 2.0,
            # Explicit key: actual gap between adjacent panels/stacked panels.
            "panel_gap_between_in": float(panel_gap_in),
...
def optimize_best_configuration(panel_list, trailer_L, trailer_W, trailer_H,
    ...
    for s in LoadingStrategy:
        # Skip WALL_FIRST on flatbed — no enclosure walls to lean panels against
...
def add_cylinder_x(fig, x0, x1, y_center, z_center, radius, color, opacity=0.65, segments=28,
                   name="", showlegend=False, theta_min=0.0, theta_max=2 * math.pi):
    """Cylinder with axis along X spanning [x0, x1].
    Cross-section lies in the Y–Z plane.

    - theta_min/theta_max allow partial cylinders (e.g., a rounded nose cap).
    """
    # Circle points (Y–Z)
    pts_y = []
    pts_z = []
    for i in range(segments):
        t = theta_min + (theta_max - theta_min) * i / (segments - 1)
        pts_y.append(y_center + radius * math.cos(t))
        pts_z.append(z_center + radius * math.sin(t))

    # Vertices: interleave x0 and x1 for each segment point
    xs, ys, zs = [], [], []
    for i in range(segments):
        ys_i = pts_y[i]
        zs_i = pts_z[i]
...
def compute_spacer_slabs_from_placements(placements, spacer_side_in, eps=1e-6):
    """Return spacer slab boxes that exist ONLY between two panels that face each other.

    We assume the packing already enforced an effective panel-to-panel gap of:
        gap_between_panels = 2 * spacer_side_in

    We generate a thin axis-aligned box occupying that gap *only* where:
      - two panels are adjacent along X or Y (side-by-side), with overlap in the other axes, OR
      - two panels are stacked along Z, with overlap in X and Y.

    No slabs are created at trailer walls/floor, and no slabs are created above the top layer
    (because they require a matching neighbor panel).
    """
    if spacer_side_in <= 0:
        return []

    gap = 2.0 * float(spacer_side_in)
    slabs = []
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            # Overlaps
            ox0 = max(ax0, bx0); ox1 = min(ax1, bx1)
            oy0 = max(ay0, by0); oy1 = min(ay1, by1)
            oz0 = max(az0, bz0); oz1 = min(az1, bz1)

            overlap_x = ox1 - ox0
            overlap_y = oy1 - oy0
            overlap_z = oz1 - oz0

            # Adjacent along X (i left of j)
            d1 = bx0 - ax1
            if abs(d1 - gap) <= max(eps, gap * 0.05) and overlap_y > eps and overlap_z > eps:
                slabs.append((ax1, oy0, oz0, d1, overlap_y, overlap_z))
                continue
            # Adjacent along X (j left of i)
            d2 = ax0 - bx1
            if abs(d2 - gap) <= max(eps, gap * 0.05) and overlap_y > eps and overlap_z > eps:
                slabs.append((bx1, oy0, oz0, d2, overlap_y, overlap_z))
                continue

            # Adjacent along Y
            d3 = by0 - ay1
            if abs(d3 - gap) <= max(eps, gap * 0.05) and overlap_x > eps and overlap_z > eps:
                slabs.append((ox0, ay1, oz0, overlap_x, d3, overlap_z))
                continue
            d4 = ay0 - by1
            if abs(d4 - gap) <= max(eps, gap * 0.05) and overlap_x > eps and overlap_z > eps:
                slabs.append((ox0, by1, oz0, overlap_x, d4, overlap_z))
                continue

            # Stacked along Z
            d5 = bz0 - az1
            if abs(d5 - gap) <= max(eps, gap * 0.05) and overlap_x > eps and overlap_y > eps:
                slabs.append((ox0, oy0, az1, overlap_x, overlap_y, d5))
                continue
            d6 = az0 - bz1
            if abs(d6 - gap) <= max(eps, gap * 0.05) and overlap_x > eps and overlap_y > eps:
                slabs.append((ox0, oy0, bz1, overlap_x, overlap_y, d6))
                continue

    # De-duplicate slabs (common in symmetric placements)
    uniq = []
    seen = set()
    for s in slabs:
        key = tuple(round(v, 4) for v in s)
        if key not in seen:
            seen.add(key)
            uniq.append(s)
    return uniq
...
def visualize(out, stress_config, color_by_stress, show_stress_markers, show_axles, floor_spec):
    ...
    # Spacer visualization (per-side)
    spacer_side = float(out.get("settings", {}).get("panel_gap_in", 0.0) or 0.0)
    spacer_legend_shown = False
    ...
    # Spacer slabs (ONLY between panel-to-panel interfaces)
    spacer_slabs = compute_spacer_slabs_from_placements(out.get("placements", []), spacer_side)
    if spacer_slabs:
        for (sx, sy, sz, sdx, sdy, sdz) in spacer_slabs:
            # Bright white, very visible
            add_solid(fig, sx, sy, sz, sdx, sdy, sdz, "#ffffff", opacity=0.35)
            add_wire(fig, sx, sy, sz, sdx, sdy, sdz, "#ffffff",
                 "Spacers" if not spacer_legend_shown else "",
                 opacity=1.0, width=5)
            spacer_legend_shown = True
    ...
    if show_axles and axle_data:
        ...
        # ── Semi-truck visualization (tractor with hood/cab/sleeper + fifth wheel) ──
        # Trailer is drawn from x∈[0, L]. Tractor is drawn in front (negative x).
        steer_x = ap.get("steer_axle_x", None)
        drive_center_x = ap.get("drive_tandem_x", None)

        # Reasonable defaults if any axle values are missing
        if steer_x is None:
            steer_x = -220.0
        if drive_center_x is None:
            drive_center_x = -110.0

        # If axle positions are in trailer coordinates (x>=0), shift tractor so the fifth wheel sits near x≈0.
        # Target a typical fifth-wheel-to-drive-center distance placing drive center around x=-60.
        _plot_shift = 0.0
        if drive_center_x > -5.0:
            _plot_shift = (-60.0) - float(drive_center_x)
        steer_x = float(steer_x) + _plot_shift
        drive_center_x = float(drive_center_x) + _plot_shift

        # Basic tractor dimensions (inches) — typical US sleeper cab (approx)
        # We build geometry relative to the (shifted) axle positions above.
        # Typical distances:
        #   bumper -> steer axle ≈ 55–60"
        #   steer axle -> drive tandem center ≈ 150–170"
        #   drive tandem center -> tractor rear ≈ 90–110"
        bumper_x = steer_x - 58.0
        rear_x   = drive_center_x + 100.0

        # Component lengths along X
        hood_len    = 72.0
        cab_len     = 78.0
        sleeper_len = max(0.0, (rear_x - (bumper_x + hood_len + cab_len)))
        sleeper_len = min(max(sleeper_len, 60.0), 90.0)  # keep sane visual proportions

        hood_end_x    = bumper_x + hood_len
        cab_end_x     = hood_end_x + cab_len
        sleeper_end_x = cab_end_x + sleeper_len

        # Width and lateral placement (center tractor in trailer width)
        tractor_w = min(102.0, W)             # typical tractor width ~102"
        tractor_y0 = (W - tractor_w) / 2.0
        ...
        # Frame / chassis
        add_solid(fig, bumper_x, tractor_y0, -chassis_h,
                  sleeper_end_x - bumper_x, tractor_w, chassis_h,
                  "#3b3b3b", opacity=0.35)
        add_wire(fig, bumper_x, tractor_y0, -chassis_h,
                 sleeper_end_x - bumper_x, tractor_w, chassis_h,
                 "#aaaaaa", "Tractor", opacity=0.75, width=3)

        
        # Curved tractor body (rounded top/sides via cylinders along X)

        tractor_yc = tractor_y0 + tractor_w / 2.0

        # Hood (rounded)
        hood_r = min(24.0, hood_h * 0.45, tractor_w * 0.28)
        hood_box_h = max(0.0, hood_h - hood_r)
        add_solid(fig, bumper_x, tractor_y0, 0.0,
          hood_len, tractor_w, hood_box_h,
          "#c0392b", opacity=0.46)
        add_cylinder_x(fig, bumper_x, hood_end_x, tractor_yc, hood_box_h, hood_r,
               "#c0392b", opacity=0.46)
        add_wire(fig, bumper_x, tractor_y0, 0.0,
         hood_len, tractor_w, hood_h,
         "#ffffff", "", opacity=0.70, width=2)

        # Windshield / cowl hint (small upper block at rear of hood)
        add_solid(fig, hood_end_x - 18.0, tractor_y0 + tractor_w * 0.10, hood_h * 0.55,
          18.0, tractor_w * 0.80, hood_h * 0.45,
          "#7f8c8d", opacity=0.22)

        # Cab (rounded)
        cab_r = min(30.0, cab_h * 0.40, tractor_w * 0.30)
        cab_box_h = max(0.0, cab_h - cab_r)
        add_solid(fig, hood_end_x, tractor_y0, 0.0,
          cab_len, tractor_w, cab_box_h,
          "#2980b9", opacity=0.46)
        add_cylinder_x(fig, hood_end_x, cab_end_x, tractor_yc, cab_box_h, cab_r,
               "#2980b9", opacity=0.46)
        add_wire(fig, hood_end_x, tractor_y0, 0.0,
         cab_len, tractor_w, cab_h,
         "#ffffff", "", opacity=0.70, width=2)

        # Sleeper (rounded, slightly narrower)
        sleeper_w = tractor_w * 0.92
        sleeper_y0 = tractor_y0 + (tractor_w - sleeper_w) / 2.0
        sleeper_yc = sleeper_y0 + sleeper_w / 2.0
        sleeper_r = min(30.0, sleeper_h * 0.40, sleeper_w * 0.30)
        sleeper_box_h = max(0.0, sleeper_h - sleeper_r)

        add_solid(fig, cab_end_x, sleeper_y0, 0.0,
          sleeper_len, sleeper_w, sleeper_box_h,
          "#2471a3", opacity=0.44)
        add_cylinder_x(fig, cab_end_x, sleeper_end_x, sleeper_yc, sleeper_box_h, sleeper_r,
               "#2471a3", opacity=0.44)
        add_wire(fig, cab_end_x, sleeper_y0, 0.0,
         sleeper_len, sleeper_w, sleeper_h,
         "#ffffff", "", opacity=0.65, width=2)

# Fifth wheel plate (at x≈0)
        add_solid(fig, -10.0, tractor_y0 + tractor_w * 0.28, -2.0, 18.0, tractor_w * 0.44, 2.0,
                  "#2c3e50", opacity=0.45)
...
def render_welcome():
    ...
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.2")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Alpha V2.2")
...
with st.sidebar:
    ...
    panel_gap_in = st.number_input(
        ...
        help=(
            "Per-panel spacer thickness on each side. "
            "The optimizer treats the effective gap *between* adjacent panels as 2× this value. "
            "HexHomes standard: 1.5 in (=> 3.0 in between panels)."
        )
    )
...
if run_btn:
    ...
    # Block WALL_FIRST on flatbed — no physical enclosure walls to lean panels against
    if is_flatbed and not auto_best and strategy == LoadingStrategy.WALL_FIRST:
        st.warning("⚠️ Wall First strategy is not available for flatbed trailers (no enclosure walls). "
                   "Switched to Gravity Layered.")
        strategy = LoadingStrategy.GRAVITY_LAYERED
        ...
        if "error" not in out:
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v22_run{st.session_state.run_count}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v22_run{st.session_state.run_count}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])

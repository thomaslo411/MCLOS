...
#V1.4 — Full packing engine rewrite. Layer-first filling, column tracking, 4 distinct loading strategies with different orientation preferences, CG & weight distribution, seed control, CSV export.
...
#NOTE: Half-Hexagon floor panels are not modeled properly due to dimension constraints, so expect them to not be loaded in the regular pod-selection simulations. 
#-given the current dimensions. If you want to see how they are modeled then you can change the dimensions to see how they work, which is not perfect and pretty wonky so this will need improvement
#-Use these dimensions for floor panels if you want to see them work decently: Long Edge: 110, Depth: 55, Thickness: 3, Short Edge: 55

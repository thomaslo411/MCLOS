...
def render_welcome():
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Beta V3")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Beta V3")
...
if out and "error" not in out:
    ...
    with d1:
        st.download_button("Download JSON", json.dumps(out, indent=2),
                           f"mclos_beta_v3_run{run_num}.json", "application/json", use_container_width=True)
    with d2:
        ...
        st.download_button("Download CSV", hdr + rows,
                           f"mclos_beta_v3_run{run_num}.csv", "text/csv", use_container_width=True)
...
def get_trapezoid_safe_zone(base_panel):
    if not base_panel.get("is_trapezoid", False):
        return None
    axis_map = base_panel.get("axis_map", ("", "", ""))
    if axis_map[2] != "T":
        return None
    bx, by, _ = base_panel["pos"]
    _, bdy, _ = base_panel["size"]
    bdx = base_panel["size"][0]
    short_edge = base_panel["short_edge"]
    long_edge = base_panel["spec_length"]
    offset = (long_edge - short_edge) / 2.0
    x_axis = axis_map[0]
    if x_axis == "L":
        return (bx + offset, by, bx + offset + short_edge, by + bdy)
    elif x_axis == "H":
        return (bx, by + offset, bx + bdx, by + offset + short_edge)
    return None
...
def stress_ok(pos, size, weight, placed, stress_config, min_support_frac=0.5, gap=0.0):
    ...
    comp = calculate_compression_stress(weight + wa, support_area)
    if comp * stress_config.safety_factor > stress_config.max_compression_psi:
        return False, f"Compression too high: {comp:.1f} psi"
    bend = calculate_bending_stress(weight + wa, pos, size, supporting_panels)
...
def apply_strategy_slot_order(strategy, floor_slots, placed, trailer, panel_size, is_flatbed=False):
    ...
    if strategy == LoadingStrategy.GRAVITY_LAYERED:
        ...
        if is_flatbed:
            center_y = trailer_W / 2.0
            return sorted(floor_slots, key=lambda s: (s[0], abs(s[1] + dy / 2.0 - center_y)))
...
def pack_panels_v14(panel_list, trailer_L, trailer_W, trailer_H,
    ...
    for panel_info in panel_list:
        ...
        for name, size, axis_map in raw:
            ...
            is_wall = (spec.panel_type == PanelType.WALL)
    ...
    for pid, panel_info in enumerate(sorted_panels):
        ...
        if walls_vertical and spec.panel_type == PanelType.WALL:
            orientations = sorted(orientations, key=lambda o: (0 if o['name'].startswith('stand') else 1))
        ...
        for orient in orientations:
            ...
            for _, (sx, sy) in indexed_slots:
                ...
                if any(panels_collide(test_pos, size, panel_info_new,
                    ...
                    # Z-probe: find first clear z above all XY-overlapping panels
                    max_z_top = 0.0
                    for p in placed:
                        px, py, pz = p["pos"]
                        pdx, pdy, pdz = p["size"]
                        if (sx < px + pdx - 0.01 and sx + dx > px + 0.01 and
                            sy < py + pdy - 0.01 and sy + dy > py + 0.01):
                            max_z_top = max(max_z_top, pz + pdz + panel_gap_in)
                    probe_z = round(max_z_top, 4)
                    if probe_z <= z or probe_z + dz > H_eff:
                        continue

                    probe_candidates = [(sx, sy)]
                    if not panel_is_floor_flat:
                        clamped_sx, clamped_sy = sx, sy
                        for p in placed:
                            px, py, pz = p["pos"]
                            pdx, pdy, pdz = p["size"]
                            p_top = pz + pdz + panel_gap_in
                            if abs(p_top - probe_z) > Z_TOL:
                                continue
                            sz = get_trapezoid_safe_zone(p)
                            if sz is None:
                                continue
                            safe_x_min, safe_y_min, safe_x_max, safe_y_max = sz
                            p_axis = p.get("axis_map", ("", "", ""))
                            if p_axis[0] == "L":
                                clamped_sx = max(clamped_sx, safe_x_min)
                                clamped_sx = min(clamped_sx, safe_x_max - dx)
                            elif p_axis[0] == "H":
                                clamped_sy = max(clamped_sy, safe_y_min)
                                clamped_sy = min(clamped_sy, safe_y_max - dy)
                        if (round(clamped_sx, 4), round(clamped_sy, 4)) != (sx, sy):
                            probe_candidates.insert(0, (round(clamped_sx, 4), round(clamped_sy, 4)))

                    probe_ok = False
                    for (pcx, pcy) in probe_candidates:
                        test_pos = (pcx, pcy, probe_z)
                        if not in_bounds_with_overhang(test_pos, size, trailer,
                                                       is_flatbed, overhang_max_in, panel_is_floor_flat):
                            continue
                        if any(panels_collide(test_pos, size, panel_info_new,
        ...
        if not placed_this and placed:
            for orient in orientations:
                ...
                for base in placed:
                    ...
                    candidates = set()
                    base_sz = get_trapezoid_safe_zone(base)
                    if base_sz is not None and not panel_is_floor_flat:
                        safe_x_min, safe_y_min, safe_x_max, safe_y_max = base_sz
                        b_axis = base.get("axis_map", ("", "", ""))
                        c1x, c1y = bx, by
                        if b_axis[0] == "L":
                            c1x = max(safe_x_min, min(c1x, safe_x_max - dx))
                        elif b_axis[0] == "H":
                            c1y = max(safe_y_min, min(c1y, safe_y_max - dy))
                        c1x = max(0.0, min(c1x, trailer_L - dx))
                        c1y = max(0.0, min(c1y, trailer_W - dy))
                        candidates.add((round(c1x, 4), round(c1y, 4)))
                        if b_axis[0] == "L":
                            center_x = safe_x_min + (safe_x_max - safe_x_min - dx) / 2.0
                            center_y = by + (bdy - dy) / 2.0
                        else:
                            center_x = bx + (bdx - dx) / 2.0
                            center_y = safe_y_min + (safe_y_max - safe_y_min - dy) / 2.0
                        center_x = max(0.0, min(center_x, trailer_L - dx))
                        center_y = max(0.0, min(center_y, trailer_W - dy))
                        candidates.add((round(center_x, 4), round(center_y, 4)))
                        if b_axis[0] == "L":
                            c3x = max(0.0, min(safe_x_max - dx, trailer_L - dx))
                            c3y = max(0.0, min(by + bdy - dy, trailer_W - dy))
                        else:
                            c3x = max(0.0, min(bx + bdx - dx, trailer_L - dx))
                            c3y = max(0.0, min(safe_y_max - dy, trailer_W - dy))
                        candidates.add((round(c3x, 4), round(c3y, 4)))
                    else:
    ...
    for i, panel in enumerate(placed):
        ...
        comp = calculate_compression_stress(panel["weight"] + wa, sa)
        bend = calculate_bending_stress(panel["weight"] + wa, panel["pos"], panel["size"], sp)
        shear = calculate_shear_stress(panel["weight"] + wa, panel["size"])
...
def visualize(out, stress_config, color_by_stress, show_stress_markers, show_axles, floor_spec,
    ...
    for p in out["placements"]:
        ...
        if p.get("is_trapezoid", False):
            ...
            p_short = p.get("short_edge", floor_spec.short_edge)
            p_long = p.get("spec_length", floor_spec.length)
            verts = make_half_hex_vertices(x, y, z, dx, dy, dz, am,
                                            p_short, p_long, flipped=fl)
...
if is_flatbed:
    overhang_max_in = st.number_input(
        ...
        value=10.0, min_value=0.0, max_value=24.0, step=1.0, key="overhang",
        help="Floor panels can extend past deck width on flatbed trailers (no walls). "
             "Max per-side overhang. 0 = no overhang allowed."
    )
    walls_vertical = st.checkbox(
        "Prioritize vertical wall panel orientation",
        value=False, key="walls_vert",
        help="When enabled, wall panels are placed standing upright first, then flat "
             "in remaining gaps. When disabled, walls are stacked flat first, then "
             "standing upright to fill remaining space."
    )
else:
    overhang_max_in = 0.0
    walls_vertical = False
...
if run_btn:
    valid_input = True
    if fS >= fL:
        st.error("Floor panel short edge must be less than long edge.")
        valid_input = False
    if rS >= rL:
        st.error("Roof panel short edge must be less than long edge.")
        valid_input = False
    ...
    if not valid_input:
        ...
        st.session_state.last_floor_spec = floor_spec
...
if out and "error" not in out:
    floor_spec_display = st.session_state.get("last_floor_spec", floor_spec)
    ...
    visualize(out, stress_config, color_by_stress, show_stress_markers, show_axles, floor_spec_display,
    ...
    with d1:
        st.download_button("Download JSON", json.dumps(out, indent=2),
                           f"mclos_beta_v4_run{run_num}.json", "application/json", use_container_width=True)
    with d2:
        ...
        st.download_button("Download CSV", hdr + rows,
                           f"mclos_beta_v4_run{run_num}.csv", "text/csv", use_container_width=True)
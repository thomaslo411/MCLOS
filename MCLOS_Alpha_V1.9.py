...
def generate_floor_slots(dx, dy, trailer_L, trailer_W, gap=0.0, align_x="front", align_y="front"):
    """
    ...
    if trailer_L + 0.01 < dx or trailer_W + 0.01 < dy:
        return []
    ...
    ny = int(math.floor((trailer_W - dy) / stride_y + 1.0 + 1e-9))
    nx = max(nx, 0)
    ny = max(ny, 0)
    ...
    slack_W = max(0.0, trailer_W - used_W)
    ...
    y0 = start_from_align(align_y, slack_W)
    ...
    for i in range(nx):
        ...
        for j in range(ny):
            y = y0 + j * stride_y
            if x + dx <= trailer_L + 0.01 and y + dy <= trailer_W + 0.01:
...
def pack_panels_v14(panel_list, trailer_L, trailer_W, trailer_H,
    ...
    for panel_info in panel_list:
        ...
        for name, size, axis_map in raw:
            ...
            dx, dy, dz = size
            floor_slots = generate_floor_slots(dx, dy, trailer[0], trailer[1], gap=panel_gap_in, align_x=align_x, align_y=align_y)
...
def render_welcome():
    ...
    if _LOGO_B64:
        ...
        with title_col:
            ...
            st.caption("HexHomes Panel Packing Optimizer  |  Alpha V1.9")
    else:
        st.title("Modular Cargo Loading Optimization Software")
        st.caption("HexHomes Panel Packing Optimizer  |  Alpha V1.9")
...
panel_gap_in = st.number_input(
    ...
    value=0.0, min_value=0.0, step=0.25, key="pgap",
    help="Extra gap inserted between adjacent panels in the floor grid. Set 0 for tight packing."
)
        ...
        if "error" not in out:
            ...
            with d1:
                st.download_button("Download JSON", json.dumps(out, indent=2),
                                   f"mclos_v19_run{st.session_state.run_count}.json", "application/json", use_container_width=True)
            with d2:
                ...
                st.download_button("Download CSV", hdr + rows,
                                   f"mclos_v19_run{st.session_state.run_count}.csv", "text/csv", use_container_width=True)
        else:
            st.error(out["error"])
